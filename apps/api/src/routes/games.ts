...
router.post('/', async (req, res) => {
  ...
  await profiles.updateAfterGame(req.authUser!.id, analyses, report);
  res.status(201).json({ game, report });
});

router.get('/', async (req, res) => {
  const games = await prisma.game.findMany({ where: { userId: req.authUser!.id }, orderBy: { createdAt: 'desc' }, take: 50 });
  res.json(games);
});

router.get('/profile', async (req, res) => {
  const profile = await prisma.learnerProfile.findUnique({ where: { userId: req.authUser!.id } });
